// proxy.recs.sellers.$productId.ts
...
export async function loader({ request, params }: any) {
  ...
  try {
    const enabled = await isRailEnabled(shop, "bestSellers");
    if (!enabled) {
    ...
    if (usage.exceeded) {
      console.warn("[Proxy: Sellers] Usage limit exceeded", { shop, ...usage });
      return Response.json({
        shop,
        productId,
        mode: "public",
    ...
    const result = await getSellerRecommendations(
      shop,
      productId,
      "public"
    );
    ...
    console.error("[Proxy: Sellers Error]", err);
    return Response.json(
