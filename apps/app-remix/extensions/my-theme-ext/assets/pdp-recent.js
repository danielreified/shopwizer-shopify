...
(() => {
  ...
  var debug8 = createDebugger("RECENT", true);
  var KEY2 = "__shopwizer_recent__v1";
  var MAX2 = 16;
  function getRawList() {
    try {
  ...
  function recordRecentlyViewed(pid, handle) {
    try {
      ...
      const list = getRawList();
      const filtered = list.filter((r3) => String(r3.pid) !== String(pid));
      filtered.push({
        pid,
        handle,
        ts: Date.now()
      });
      const toSave = filtered.slice(-MAX2);
      localStorage.setItem(KEY2, JSON.stringify(toSave));
      debug8.log("Recorded:", handle, "| Total:", toSave.length);
    } catch (err) {
      debug8.warn("Failed to record", err);
    }
  }

  // src/pdp-recent.tsx
  var debug9 = createDebugger("RECENT", true);
  async function fetchIntegrations(shop) {
    try {
      const res = await fetch(`/apps/sw/integrations/${shop}`);
      if (!res.ok) {
        debug9.warn("Failed to fetch integrations", res.status);
        return {};
      }
      const data = await res.json();
      return data.integrations || {};
    } catch (err) {
      debug9.warn("Error fetching integrations", err);
      return {};
    }
  }
  async function loadRecent(settings) {
    debug9.log("Loading recently viewed", settings);
    ...
    if (currentProductId && currentHandle) {
      ...
      debug9.log("Recorded current product:", currentHandle);
    }
    const recentItems = getRecentlyViewed();
    if (!recentItems.length) {
      debug9.log("No recently viewed items");
      return { products: [], slate_id: null, integrations: {} };
    }
    debug9.log("Recent items from storage:", recentItems);
    const filteredItems = recentItems.filter((item) => String(item.pid) !== String(currentProductId)).slice(0, limit);
    if (!filteredItems.length) {
      debug9.log("No items after filtering current product");
      return { products: [], slate_id: null, integrations: {} };
    }
      ...
      Promise.all(
        filteredItems.map(async (item) => {
          try {
            ...
            debug9.warn("Failed to fetch product:", item.handle, err);
            return null;
          }
        })
      ),
    ...
    debug9.log("Loaded products:", validProducts.length);
    debug9.log("Loaded integrations:", integrations);
    return {
  ...
  function RecentWidget({ el }) {
    return /* @__PURE__ */ u3(Widget, { el, loader: loadRecent });
  }
