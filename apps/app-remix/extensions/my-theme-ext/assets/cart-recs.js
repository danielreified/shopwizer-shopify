...
(() => {
  ...
  // src/cart-recs.tsx
  var debug8 = createDebugger("CART_RECS", true);
  ...
  async function cartLoader(settings) {
    debug8.log("cartLoader() settings:", settings);
    const shop = settings?.shop;
    const limit = Number(settings.productsToShow || 8);
    if (!shop) {
      debug8.warn("No shop domain");
    ...
    try {
      ...
      if (cartItems.length === 0) {
        debug8.log("Cart is empty");
        return { products: [], slate_id: null, integrations: {} };
      }
      const productIds = [...new Set(cartItems.map((item) => String(item.product_id)))];
      debug8.log("Cart product IDs:", productIds);
      const url = `/apps/sw/recs/${shop}/cart?ids=${productIds.join(",")}`;
      debug8.log("Fetching", url);
      ...
      debug8.log("API returned", results.length, "results");
      ...
      debug8.warn("ERROR:", err);
      return { products: [], slate_id: null, integrations: {} };
    }
  }
  function CartRecsComponent({ el }) {
    return /* @__PURE__ */ u3(Widget, { el, loader: cartLoader });
  }
  debug8.log("register mount for cart_recs");
  mountByRecommender("cart_recs", (args) => CartRecsComponent(args));
  var cart_recs_default = CartRecsComponent;
})();
