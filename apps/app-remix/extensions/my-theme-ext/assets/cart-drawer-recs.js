...
(() => {
  ...
  function debounce(fn2, ms) {
    ...
    return ((...args) => {
      ...
      timeout = setTimeout(() => fn2(...args), ms);
    });
  }
  function formatMoney2(cents, format) {
    const amount = (cents / 100).toFixed(2);
    return format.replace(/^["']|["']$/g, "").replace("{{amount}}", amount).replace("{{amount_no_decimals}}", Math.round(cents / 100).toString()).replace("{{amount_with_comma_separator}}", amount.replace(".", ","));
  }
  function CartDrawerWidget({ settings, cartProductIds }) {
    const [products, setProducts] = d2([]);
    const [loading, setLoading] = d2(true);
    const [slateId, setSlateId] = d2(null);
    ...
    y2(() => {
      ...
      setLoading(true);
      fetch(`/apps/sw/recs/${settings.shop}/cart?ids=${cartProductIds.join(",")}`).then((res) => res.ok ? res.json() : Promise.reject()).then(async (data) => {
        setSlateId(data.slate_id || null);
        const handles = (data.results || []).slice(0, settings.productsToShow).map((r3) => r3.handle);
        const fetched = await Promise.all(handles.map(async (handle) => {
          ...
          return {
            ...
            price: formatMoney2(product.price || 0, settings.moneyFormat),
            compareAtPrice: product.compare_at_price ? formatMoney2(product.compare_at_price, settings.moneyFormat) : null,
    ...
    const handleQuickBuy = async (e3, product) => {
      e3.preventDefault();
      e3.stopPropagation();
      try {
        const prodData = await fetch(`/products/${product.handle}.js`).then((r3) => r3.json());
    ...
    return /* @__PURE__ */ u3("div", { className: "sw-drawer-recs", children: [
      /* @__PURE__ */ u3("div", { className: "sw-drawer-recs__title", children: settings.headingTitle }),
      /* @__PURE__ */ u3("div", { className: "sw-drawer-recs__grid", children: Array.from({ length: slotCount }).map((_3, index) => {
        const product = loading ? void 0 : products[index];
        if (!product) {
          return /* @__PURE__ */ u3("div", { className: "sw-drawer-recs__item sw-drawer-recs__item--skeleton", children: [
            /* @__PURE__ */ u3("div", { className: "sw-drawer-recs__image-wrap sw-skel-shimmer", style: { "--aspect-ratio": settings.imageRatio } }),
            /* @__PURE__ */ u3("div", { className: "sw-drawer-recs__info", children: [
              /* @__PURE__ */ u3("div", { className: "sw-skel-line sw-skel-shimmer" }),
              settings.showPrice && /* @__PURE__ */ u3("div", { className: "sw-skel-line sw-skel-line--short sw-skel-shimmer" })
            ] })
          ] }, index);
        }
        return /* @__PURE__ */ u3(
          "a",
          {
            ...
            children: [
              /* @__PURE__ */ u3("div", { className: "sw-drawer-recs__image-wrap", style: { "--aspect-ratio": settings.imageRatio }, children: [
                product.imageUrl && /* @__PURE__ */ u3("img", { src: `${product.imageUrl}&width=200`, alt: product.title, loading: "lazy" }),
                /* @__PURE__ */ u3("button", { className: "sw-drawer-recs__quick-buy", onClick: (e3) => handleQuickBuy(e3, product), "aria-label": "Add to cart", children: /* @__PURE__ */ u3("svg", { width: "14", height: "14", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.5", children: [
              ...
              /* @__PURE__ */ u3("div", { className: "sw-drawer-recs__info", children: [
                /* @__PURE__ */ u3("div", { className: "sw-drawer-recs__product-title", children: product.title }),
                settings.showPrice && /* @__PURE__ */ u3("div", { className: "sw-drawer-recs__price-wrap", children: [
                  /* @__PURE__ */ u3("span", { className: "sw-drawer-recs__price", children: product.price }),
                  product.compareAtPrice && /* @__PURE__ */ u3("span", { className: "sw-drawer-recs__compare-price", children: product.compareAtPrice })
                ] })
              ] })
            ]
          },
  ...
  var CartDrawerWatcher = class {
    ...
    start() {
      this.observer = new MutationObserver((mutations) => {
        const isOurChange = mutations.every((m3) => m3.target.closest?.("#sw-cart-drawer-recs"));
    ...
    async checkAndMount() {
      ...
      if (!target) return;
      const cart = await fetch("/cart.js").then((r3) => r3.json()).catch(() => ({ items: [] }));
      const cartIds = [...new Set((cart.items || []).map((i3) => String(i3.product_id)))].join(",");
      if (!cartIds) {
        if (this.container) {
          G(null, this.container);
          this.container = null;
      ...
      G(/* @__PURE__ */ u3(CartDrawerWidget, { settings: this.settings, cartProductIds: cartIds.split(",") }), container);
    }
    destroy() {
      ...
      if (this.container) G(null, this.container);
    }
  };
  function init() {
